...
import { getConsultation, completeConsultation } from "@/backend/services/consultationService";
import { completeConsultationSchema } from "@/backend/validations/consultationValidation";
...
export async function POST(request, { params }) {
  try {
    ...
    await completeConsultation(authUser, consultationId, validatedData);

    const updated = await getConsultation(authUser, consultationId);

    // Strip private notes if not authorized
    const sanitized = sanitizeConsultation(updated);
